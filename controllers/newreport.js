  ...
  jsonStream.on('end', () => {
    ...
    if (branch) {
      ...
      if (branchData[branch]) {
        ...
        response.unshift({ branch_name: branchName, total_procedure_service_amount: totalBranchSum, service_type: "Procedural Service" });
      } else {
        response.push({
          // branch_name: branchInfo.branch_name,
      ...
      masterBranches.forEach((branchInfo) => {
        ...
        const totalSumForBranch = branchTotalAmount + totalProcedureServiceAmountForBranch;
      ...
      const totalSum = Object.values(totalBranchSum).reduce((acc, val) => acc + val, 0);
...
const patient_advance = (req, res) => {
      ...
      const response = [];
        ...
        if (branch) {
          if (branchData[branch]) {
              ...
              response.unshift({ branch_name: branchName, total_total_amount: totalBranchSumValue,"service_type": "Rental / Membership" });
          } else {
              return res.status(404).json({ message: 'No data found for the given branch' });
          }
      } else {
        ...
        const totalTotalAmount = Object.values(totalBranchSum)
          ...
          .toFixed(2);
        response.unshift({ total_total_amount: totalTotalAmount, "service_type": "Rental / Membership" });
      }
  ...
  const staff_extra_service = (req, res) => {
    ...
    jsonStream.on('end', () => {
        ...
        for (const branchId in branchData) {
          const branchInfo = masterBranches.find((branch) => branch.id === branchId);
            const branchResponse = { branch_name: branchInfo ? branchInfo.branch_name : 'N/A', total_branch_staff_extra_amount: totalBranchSum[branchId],"service_type": "Staff Extra Service", data: branchData[branchId].data };
            response.push(branchResponse);
        }
